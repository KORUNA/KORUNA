...
unsigned int nStakeMaxAge = 2000000000; // No max age         
...
const string strMessageMagic = "Koruna Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    int64_t nSubsidy;
	int year = 60*24*365;
	int yearselapsed = nHeight/year;
	
	int interest = 7 - yearselapsed;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Koruna", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Czechoslovakia - 1918-10-28";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1451610061;
        }
        else
        {
